...
select
  ...
  -------------------------
from
  mat_pre_clear.msd001motherdemog md
  left join mat_pre_clear.msd101pregnancybooking pb on md.Person_ID_Mother = pb.Person_ID_Mother
  left join mat_pre_clear.msd401babydemographics bd on bd.UniqPregID = pb.UniqPregID
  left join mat_pre_clear.msd103datingscan ds on bd.UniqPregID = ds.UniqPregID
  left join mat_pre_clear.msd301labourdelivery ld on bd.UniqPregID = ld.UniqPregID
  left join mat_pre_clear.msd004overseasvischargcat os on md.UniqSubmissionID =  os.UniqSubmissionID
  left join mat_pre_clear.msd402neonataladmission na on pb.UniqPregID = na.UniqPregID
  left join mat_pre_clear.msd405careactivitybaby cab on bd.Person_ID_baby = cab.Person_ID_Baby and (cab.birthweight is not null or cab.MasterSnomedCTObsTerm = 'Birth weight (observable entity)')
where (md.NHSNumberMother =  and abs(datediff(pb.EDDAGreed,'')) < 30)
order by
  1;
